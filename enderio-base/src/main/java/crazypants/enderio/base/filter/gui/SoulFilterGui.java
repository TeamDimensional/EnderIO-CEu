...
public class SoulFilterGui extends AbstractFilterGui {
  ...
  private final @Nonnull SoulFilter filter;
  ...
  public SoulFilterGui(@Nonnull InventoryPlayer playerInv, @Nonnull ContainerFilter filterContainer, int xOffset, int yOffset, TileEntity te,
      @Nonnull IItemFilter filterIn) {
    super(playerInv, filterContainer, te, filterIn, "soul_filter_normal", "soul_filter_big");
    this.xOffset = xOffset;
    this.yOffset = yOffset;

    filter = (SoulFilter) filterIn;
  ...
  public void renderCustomOptions(int top, float par1, int par2, int par3) {
    ...
    NNList<CapturedMob> souls = filter.getSouls();
    for (GhostSlot slot : getGhostSlotHandler().getGhostSlots()) {
      if (slot instanceof SoulFilterGhostSlot) {
        int slotno = slot.getSlot();
        if (slotno < souls.size()) {
          String displayName = souls.get(slotno).getDisplayName();
